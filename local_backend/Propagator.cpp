...
State get_derivatives(const State& state, double t, bool nbody_enabled) {
    ...
    if (nbody_enabled) {
        Vector3d earth_helio = get_earth_helio_pos(t);
        vector<Body> bodies = {
            {"Sun", MU_SUN, -earth_helio},
            {"Moon", MU_MOON, get_moon_geo_pos(t)},
            {"Jupiter", MU_JUPITER, get_jupiter_helio_pos(t) - earth_helio}
        };
    ...
    if (altitude < 200.0 && altitude > 0) {
        double rho = RHO0 * exp(-(altitude * 1000.0) / (SH * 1000.0));
        double v_rel_ms = v_vec.norm() * 1000.0;
        Vector3d v_vec_ms = v_vec * 1000.0;
        Vector3d acc_drag_ms2 = -0.5 * CD * AM * rho * v_rel_ms * v_vec_ms;
        acc += acc_drag_ms2 / 1000.0;
    }
...
int main(int argc, char* argv[]) {
    ...
    double default_alt = 400.0;
    double default_v = sqrt(MU_EARTH / (RE + default_alt));
    state << (RE + default_alt), 0.0, 0.0,
             0.0, default_v * cos(51.6 * M_PI/180.0), default_v * sin(51.6 * M_PI/180.0);
    ...
    } else if (argc >= 8 && string(argv[1]) != "preview") {
        state << stod(argv[1]), stod(argv[2]), stod(argv[3]),
                 stod(argv[4]), stod(argv[5]), stod(argv[6]);
        nbody_enabled = stoi(argv[7]) == 1;
    }

    double t = 0.0;
    double dt = preview_mode ? 100.0 : 10.0;
    ...
    if (preview_mode) {
        ...
        for (int i = 0; i < 8640; ++i) { // 10 days at 100s steps
            ...
            if (i < 8639) cout << ",";
            if (state.segment<3>(0).norm() <= RE) break;
        }
        cout << "]" << endl;
        return 0;
    }
