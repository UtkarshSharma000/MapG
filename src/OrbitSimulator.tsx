...
const Moon = React.memo(function Moon({
  ...
  const orbitPoints = useMemo(() => {
    ...
    for (let i = 0; i <= 100; i++) {
      ...
      pts.push(
        new THREE.Vector3(x * orbitScale, y * orbitScale, z * orbitScale),
      );
    }
    return pts;
  ...
  return (
    <group>
      ...
      >
        {data.name === "Earth" &&
          launchParams &&
          launchParams.launchPlanet === "Earth" &&
          (!launchParams.targetPlanet ||
            launchParams.targetPlanet === "Earth") &&
          (!launchParams.missionLegs ||
            launchParams.missionLegs.length === 0) && (
            <GhostPath
...
function GhostPath({
  ...
  const [interceptPoint, setInterceptPoint] = useState<THREE.Vector3 | null>(
    null,
  );
  ...
  const calculateInterplanetaryPath = useCallback(async () => {
    ...
    try {
      ...
      simDurationRef.current =
        data.usedDuration || data.arrivalTime - payload.launchTime;
      ...
      const [ix, iy, iz] = propagateOrbit(
        targetPlanet.elements,
        data.arrivalTime,
      );
      setInterceptPoint(
  ...
  useEffect(() => {
    ...
    const isInterplanetary =
      (launchParams.targetPlanet &&
        launchParams.targetPlanet !== (launchParams.launchPlanet || "Earth")) ||
      !!launchParams.missionLegs;
    ...
    if (legsChanged || targetChanged) {
      ...
      const isPlanningReturn =
        launchParams.launchPlanet &&
        launchParams.launchPlanet !== "Earth" &&
        launchParams.targetPlanet === "Earth";
      if (isPlanningReturn) {
    ...
    const fetchPreview = async () => {
      try {
        ...
        if (res.data.path) {
          ...
          const orbitPoints = res.data.path
            .filter((p: any) => p && Array.isArray(p) && p.length >= 3)
            .map(
              (p: number[]) =>
                new THREE.Vector3(
  ...
  useFrame((state, delta) => {
    ...
    if (isInterplanetary) {
      ...
      if (pct_tof < 0.05) {
        ...
        const endBurnPct = Math.max(
          0,
          ((MAX_DV - requiredDVRef.current) / MAX_DV) * 100.0,
        );
        fuelRef.current =
        ...
        const finalFuelPct = Math.max(
          0,
          ((captureInfoRef.current.remainingDeltaV ?? 0) / MAX_DV) * 100.0,
        );
    ...
    let targetName = launchParams.targetPlanet;
    if (targetName === "Earth" && launchParams.launchPlanet === "Earth")
      targetName = "";
    if (launchParams.missionLegs && launchParams.missionLegs.length > 0) {
      const legs = launchParams.missionLegs;
      const lastDestId = legs[legs.length - 1].destId;
    ...
    const elapsed =
      globalTimeRef.current - (launchTimeRef.current || globalTimeRef.current);
    const pct_tof = elapsed / transferTimeRef.current;

    if (
      pct_tof >= 1.0 &&
      targetPlanet &&
      (targetPlanet.name !== "Earth" || launchParams.launchPlanet !== "Earth")
    ) {
      const time = globalTimeRef.current;
      ...
      shuttleRef.current.userData.nextPos =
        shuttleRef.current.userData.nextPos || new THREE.Vector3();
      shuttleRef.current.userData.nextPos.set(
        tpX * POS_SCALE + nox,
        tpZ * POS_SCALE,
        -tpY * POS_SCALE + noz,
      );
  ...
  return (
    <group>
      <Line
        ...
        transparent
        opacity={
          reachedDestination ? 0.0 : launchParams?.isLaunched ? 0.7 : 0.4
        }
        dashed={true}
        dashScale={50}
        dashSize={1}
...
const OrbitingShuttle = React.memo(function OrbitingShuttle({
  ...
  const planet = PLANETS.find(
    (p) => p.name.toUpperCase() === planetName.toUpperCase(),
  );
  ...
  useFrame(() => {
    ...
    const nox = Math.cos(nextAng) * orbitalRadius;
    const noz = Math.sin(nextAng) * orbitalRadius;
    ref.current.userData.nextLocalPos =
      ref.current.userData.nextLocalPos || new THREE.Vector3();
    ref.current.userData.nextLocalPos.set(
      pX * POS_SCALE + nox,
      pZ * POS_SCALE,
      -pY * POS_SCALE + noz,
    );
    ...
    <group ref={ref}>
      ...
      <Html distanceFactor={20} position={[0, 0.5, 0]}>
        <div className="bg-black/80 px-2 py-0.5 rounded border border-cyan-500/30 flex flex-col gap-0 shadow-lg min-w-[70px]">
          ...
          <div className="text-[8px] text-cyan-400 font-mono font-bold tracking-tight uppercase whitespace-nowrap">
            {planetName.toUpperCase()} ORBIT
          </div>
        </div>
      </Html>
    </group>
...
const ArchivedShuttle = React.memo(function ArchivedShuttle({
  mission,
  globalTimeRef,
  onDoubleClick,
}: {
...
const SystemEngine = React.memo(function SystemEngine({
  ...
  const presetsRef = useRef<
    Record<number, { position: THREE.Vector3; target: THREE.Vector3 }>
  >({});
  ...
  useFrame((state, delta) => {
    ...
    if (isCinematic && cinematicScrollRef) {
      ...
      smoothedScrollRef.current +=
        (rawS - smoothedScrollRef.current) * lerpSpeed;
      ...
      const pE = propagateOrbit(
        PLANETS.find((p) => p.name === "Earth")!.elements,
        globalTimeRef.current,
      );
      tempTpE.set(pE[0] * POS_SCALE, pE[2] * POS_SCALE, -pE[1] * POS_SCALE);

      const pM = propagateOrbit(
        PLANETS.find((p) => p.name === "Mars")!.elements,
        globalTimeRef.current,
      );
      ...
      if (s < 0.2) {
        ...
        tempCamPos
          .set(0, 400, 0)
          .lerp(
            tempE1.set(tempTpE.x + 15, tempTpE.y + 5, tempTpE.z + 15),
            ease,
          );
        ...
        const ease = t * t * (3 - 2 * t);
        tempCamPos
          .set(tempTpE.x + 15, tempTpE.y + 5, tempTpE.z + 15)
          .lerp(
            tempM1.set(tempTpM.x + 20, tempTpM.y + 10, tempTpM.z + 20),
            ease,
          );
      ...
      if (controlsRef.current) {
        ...
        const targetName =
          spectatedLabel || (selectedTarget ? selectedTarget.name : "Sun");
        ...
        if (lastActiveTargetPosRef.current && !targetChanged) {
          tempDisplacement
            .copy(tempActiveTargetPos)
            .sub(lastActiveTargetPosRef.current);
        }
  ...
  const showOrbitingShuttle =
    !launchParams?.isLaunched &&
    launchParams?.missionStatus &&
    launchParams.missionStatus.endsWith("_ORBIT") &&
    launchParams.missionStatus !== "EARTH_ORBIT";
  const orbPlanetName = showOrbitingShuttle
    ? launchParams.missionStatus.replace("_ORBIT", "")
    : "";
        ...
        <div className="absolute top-24 left-8 pointer-events-auto flex flex-col gap-2">
          ...
          <div className="flex items-center gap-4 py-2 opacity-80 pl-16">
            <div className="flex flex-col">
              <span className="text-[9px] text-[#8ab4f8] font-mono tracking-[0.25em] uppercase pb-1">
                Epoch Reference
              </span>
            ...
            <div className="px-3 py-1 bg-[#0a0a0c]/95 border border-white/10 text-white/50 text-[9px] font-mono tracking-widest rounded-lg flex items-center gap-2 self-start">
              ...
              {spectatedLabel
                ...
                : selectedTarget
                  ? selectedTarget.name.toUpperCase()
                  : "SOL"}
              <span className="ml-2 text-[8px] opacity-30">
                (DRAG TO UNLOCK)
              </span>
            </div>
          )}
      ...
      {isCalculating && (
        <div className="absolute inset-0 z-[100] flex items-center justify-center pointer-events-none">
          <div className="text-cyan-400 font-mono tracking-widest text-[10px] uppercase bg-black/80 px-4 py-2 border border-cyan-500/30 rounded shadow overflow-hidden relative">
            ...
            <TextShimmer
              duration={1.5}
              className="[--base-color:var(--color-cyan-300)] [--base-gradient-color:var(--color-cyan-100)]"
            >
