...
const PRESETS: Record<string, Partial<ConfigType>> = {
...
const mergeConfigs = (...configs: any[]): ConfigType => 
  configs.reduce((acc, c) => ({ ...acc, ...c }), {}) as ConfigType;
...
const useResponsiveDimension = (responsive: boolean, config: ConfigType, key: 'height' | 'width') => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      let v = config[key];
      if (w <= 480) {
        if (key === 'height' && config.mobileHeight) v = config.mobileHeight;
        if (key === 'width' && config.mobileWidth) v = config.mobileWidth;
      } else if (w <= 768) {
        if (key === 'height' && config.tabletHeight) v = config.tabletHeight;
        if (key === 'width' && config.tabletWidth) v = config.tabletWidth;
      } else if (w <= 1024) {
        if (key === 'height' && config.desktopHeight) v = config.desktopHeight;
        if (key === 'width' && config.desktopWidth) v = config.desktopWidth;
      }
      setValue(v);
    };
...
const GradualBlurMemo = React.memo(GradualBlurComponent) as React.MemoExoticComponent<React.FC<GradualBlurProps>> & {
