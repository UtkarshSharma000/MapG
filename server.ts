...
const planOptimizedFlight: FunctionDeclaration = {
  ...
  description:
    "Calculates the most optimal interplanetary trajectory flight path between the origin planet and the destination planet using the orbital optimizer engine, showing the resulting path to the user.",
  parameters: {
    type: Type.OBJECT,
    properties: {
      originPlanet: {
        type: Type.STRING,
        description:
          "The name of the origin planet (e.g., Venus, Earth, Mars). Choose one from: Mercury, Venus, Earth, Mars, Jupiter, Saturn, Uranus, Neptune.",
      },
      destinationPlanet: {
        type: Type.STRING,
        description:
          "The name of the target planet (e.g., Mars, Venus, Earth). Choose one from: Mercury, Venus, Earth, Mars, Jupiter, Saturn, Uranus, Neptune.",
      },
    },
...
const launchSimulation: FunctionDeclaration = {
  ...
  description:
    "Engages the planned orbital flight path, igniting space shuttle engines and starting the journey along the calculated target trajectory.",
};

const abortSimulation: FunctionDeclaration = {
  name: "abort_simulation",
  description:
    "Aborts the current active flight path, stopping space travel and resetting simulation propagation instantly.",
};
  ...
  parameters: {
    ...
    properties: {
      multiplier: {
        type: Type.NUMBER,
        description:
          "The warp speed multiplier. E.g., 0 to pause, 1 for real speed, 86400 for 1 day/sec, 2592000 for 30 days/sec.",
      },
    },
    required: ["multiplier"],
  },
...
const setSimulationTarget: FunctionDeclaration = {
  ...
  description:
    "Focuses the camera target focus of the planetary viewport to the specified planet celestial body.",
  parameters: {
    type: Type.OBJECT,
    properties: {
      targetPlanet: {
        type: Type.STRING,
        description:
          "The name of the target planet to focus. Must be one of: Sol (Sun), Mercury, Venus, Earth, Mars, Jupiter, Saturn, Uranus, Neptune.",
      },
    },
...
const planReturnFlight: FunctionDeclaration = {
  ...
  description:
    "Sets up and calculates the return trajectory flight path back to planet Earth from the current orbiting destination.",
};
...
async function startServer() {
  ...
  new Promise((resolve) => {
    try {
      ...
      execSync("python3 -m pip install -r local_backend/requirements.txt", {
        stdio: "inherit",
      });
      resolve(true);
    } catch (error) {
  ...
  app.get("/api/telemetry", async (req, res) => {
    try {
      ...
      res
        .status(503)
        .json({ status: "engine_starting", error: "FastAPI not ready" });
    }
  });
  ...
  app.post(
    ...
    (req: express.Request, res: express.Response) => {
      try {
        ...
        if (!userId || !design) {
          res.status(400).json({ error: "Missing userId or design" });
          return;
        }

        let designs: Record<string, any[]> = {};
        if (fs.existsSync(DESIGNS_FILE)) {
          designs = JSON.parse(fs.readFileSync(DESIGNS_FILE, "utf8"));
        }
    ...
    try {
      ...
      if (!apiKey) {
        ...
        res.write(
          JSON.stringify({
            response:
              "⚠️ **SYSTEM WARNING**: `GEMINI_API_KEY` is not configured in your `.env` file. Please click **Settings > Secrets** in the top right menu to add your key or configure it in the `.env` file at the root, then retry to activate the celestial co-processor.",
          }) + "\n",
        );
        res.end();
        return;
      ...
      if (!res.headersSent) {
        res
          .status(502)
          .json({ error: err.message || "Failed to route your GenAI stream" });
      } else {
        res.write(
          JSON.stringify({ error: err.message || "Route aborted mid-stream" }) +
            "\n",
        );
        res.end();
      }
    ...
    (
      ...
      if (req.path.startsWith("/api")) {
        ...
        res
          .status(500)
          .send(
            "<h1>Internal Server Error</h1><p>Something went wrong on our end.</p>",
          );
      }
    },
  );
